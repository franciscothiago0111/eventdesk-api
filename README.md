...
```
src/
├── domain/            # Aggregates, value objects, domain events, repository interfaces — no framework/Prisma imports
...
│   ├── auth/ event/ registration/ check-in/
...
`.env.example` documents `DATABASE_URL`, `REDIS_HOST`/`REDIS_PORT`, `JWT_SECRET`, `JWT_EXPIRES_IN`, `PORT`. Defaults point at the ports below — no edits needed for local dev.
...
- `yarn test:cov` — unit test coverage report. Current baseline: **34.26%** statements (30.88% branches, 46.15% functions, 34.83% lines).
...
NestJS · Prisma 7 (Postgres) · Redis + BullMQ · Socket.io · Passport JWT · class-validator · Jest + Supertest · `@nestjs/swagger`
